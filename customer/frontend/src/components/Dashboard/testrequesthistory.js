...
const TestRequestHistory = () => {
    ...
    const [customer_id, setCustomerId] = useState(''); // Rename setcustomer_id to setCustomerId

    useEffect(() => {
        const customerIdFromStorage = localStorage.getItem('customerId');
        if (customerIdFromStorage) {
            setCustomerId(customerIdFromStorage); // Set customer_id state with the value from localStorage
        }
    }, []);
    ...
    }, [customer_id]); // Add customer_id to the dependency array
